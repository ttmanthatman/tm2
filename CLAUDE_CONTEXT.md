...
- 部署: VPS (45.62.123.222) + PM2 + Nginx，通过 update.sh 从 GitHub 拉取更新
- 当前版本: v0.4.3 (含动态气泡 v1 + 语音修复)
...
| 外观/通知/时区/气泡样式设置 (含 bubble_dynamic_on) | routes/settings.js |
...
| 全局响应式状态 | js/store.js |
| 陀螺仪姿态感知 (动态气泡用) | js/gyro.js |
| API 调用 + 外观应用 + 气泡 3D 引擎 + 动态光照引擎 | js/api.js |
| Socket.IO 客户端 | js/socket-client.js |
| Web Push 客户端 | js/push-client.js |
| 工具函数 (转义/时间/清理) | js/utils.js |
| 可扩展表情注册表 | js/emoji.js |
| 管理员弹窗方法 (含气泡样式草稿/保存/陀螺仪测试) | js/modals/admin-methods.js |
...
| 消息气泡 + 气泡 CSS 变量 + 2D/3D 模式规则 + 输入框 + 接龙卡片 + 动画 | chat.css |
| 所有弹窗 | modals.css |
...
```
socket.io.min.js (CDN)
vue.global.prod.js (CDN)
→ emoji.js → utils.js → store.js → gyro.js → api.js
→ socket-client.js → push-client.js
→ modals/admin-methods.js → modals/files-admin-methods.js
→ app.js
```

gyro.js 必须在 api.js 之前 (api.js 引用 `window.Gyro`)。
...
applyAppearance(d)  →  _applyBubbleStyle(d)  (api.js)
  ↓ 计算 CSS 变量                ↓ 若 bubble_dynamic_on='1' && style='3d'
  ↓                              ↓ 缓存配置 → _startDynamicBubble()
  ↓                              ↓ Gyro.onTilt → 每帧重建 CSS 变量
...
| 颜色 | bubble_my_color2 | hex | #667eea | 我方副色 (渐变/3D 用) |
...
| 渐变 | bubble_gradient_angle | 0-360 | 135 | 2D 单泡渐变角度 |
| 3D | bubble_3d_intensity | 10-100 | 60 | 整体 3D 强度 (%) |
| 3D | bubble_3d_bevel | 0-100 | 50 | 倒角宽度 (%) |
| 动态 | bubble_dynamic_on | 0/1 | 0 | 陀螺仪动态光照开关 (仅 3D 模式) |
...
| --b-angle | 2D 单泡渐变角度 |
| --b-my-3d-bg, --b-my-3d-shadow, --b-my-3d-border | 我方 3D 预计算值 (动态模式下每帧重写) |
| --b-ot-3d-bg, --b-ot-3d-shadow, --b-ot-3d-border | 对方 3D 预计算值 (动态模式下每帧重写) |
...
跨 4 个文件: gyro.js → api.js → admin-methods.js → app.js

### 当前状态 (v1 — 归一化倾斜值)

gyro.js 输出 `tiltX / tiltY ∈ [-1, +1]`，由 `MAX_TILT=30°` 归一化，自适应基线 (中位数采样 + 慢漂移跟随)，lerp 平滑。api.js 用 `_dynBuild3D(col, cache, tx, ty)` 每帧偏移镜面高光位置、渐变角度、倒角 inset 和外阴影方向，重写 6 个 CSS 变量。

### 已知待改进
- 角度被截断到 ±1（±30°），大幅度运动效果打折
- 不读磁力计 (alpha/罗盘)，转身面向不同方向无变化
- 未来计划: 改用 sin/cos 全值域 + deviceorientationabsolute + 罗盘旋转矩阵 (已做过一版但 revert 了，见 git 历史 e64952a)

### 数据流

```
deviceorientation 事件 (beta, gamma)
  ↓ 中位数基线 + 归一化 ±30° → ±1 + lerp 平滑
gyro.js 输出 tiltX, tiltY
  ↓ Gyro.onTilt(callback)
api.js _onGyroTilt(tx, ty)
  ↓ _dynBuild3D: 镜面位置/渐变角度/倒角/阴影 全部偏移
  ↓ setProperty → 6 个 --b-*-3d-* CSS 变量
chat.css 消费变量 → 气泡外观实时变化
```
...
| 文件 | 职责 |
|------|------|
| gyro.js | DeviceOrientation 监听, iOS 权限, 自适应基线, lerp 平滑, 输出 tiltX/tiltY |
| api.js | `_buildDynCache` 预计算静态部分; `_dynBuild3D` 每帧重建 CSS; `_startDynamicBubble` / `_stopDynamicBubble` 生命周期 |
| admin-methods.js | `_gyroInitState` 能力检测; `doRequestGyroPerm` iOS 权限; `doStartGyroCapture` / `doStopGyroCapture` 测试回调 |
| app.js | 3D 设置区内「动态光照」checkbox + 陀螺仪测试 UI + 实时数据显示 |

## 语音录制注意事项

- 录制使用 `_cancelled` 标志位防止取消后仍发送 (MediaRecorder.stop 是异步的, onstop 回调里检查标志)
